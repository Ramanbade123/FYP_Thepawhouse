...
import { 
  ...
  CheckCircle, PawPrint, ArrowRight
} from 'lucide-react';
...
const Contact = () => {
  ...
  const contactDetails = [
    { icon: Phone,  title: "Call Us",          description: "Direct line to our rescue team", value: "+977 9876543210",        link: "tel:+9779876543210" },
    { icon: Mail,   title: "Email Support",    description: "Send us your questions",          value: "support@thepawhouse.com", link: "mailto:support@thepawhouse.com" },
    { icon: MapPin, title: "Adoption Center",  description: "Visit our furry friends",         value: "Bhaktapur, Nepal",       link: "#" },
  ];
    ...
    <div className="bg-[#EDEDED] min-h-screen">
      ...
      <section className="relative py-20 lg:py-28 overflow-hidden bg-gradient-to-br from-[#063630] to-[#05221C] text-white">
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md px-4 py-2 rounded-full mb-6 border border-white/20">
          ...
          <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-lg md:text-xl text-white/80 max-w-2xl mx-auto">
      ...
      <section className="py-20 lg:py-32 container mx-auto px-4" ref={ref}>
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-16">
          ...
          <div className="lg:col-span-5 space-y-8">
            ...
            <div className="space-y-6">
              {contactDetails.map((detail, idx) => (
                <motion.a key={idx} href={detail.link}
                  initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: idx * 0.1 + 0.3 }}
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="font-bold text-[#008737] text-lg break-words leading-tight">{detail.value}</p>
                    </div>
          ...
          {/* Contact Form */}
          <motion.div initial={{ opacity: 0, x: 30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }} className="lg:col-span-7">
            <div className="bg-white rounded-[40px] p-8 lg:p-12 shadow-2xl border border-white relative overflow-hidden">
              {formState === 'success' && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 bg-white/95 backdrop-blur-sm z-50 flex flex-col items-center justify-center text-center p-8">
                  <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mb-6">
                    <CheckCircle className="h-10 w-10 text-green-600" />
                  </div>
                  <h3 className="text-3xl font-bold mb-4">Message Sent!</h3>
                  <p className="text-gray-600 text-lg max-w-sm">Thank you for reaching out. A member of our team will get back to you shortly.</p>
                  <button onClick={() => setFormState('idle')} className="mt-8 text-[#008737] font-bold hover:underline">Send another message</button>
                </motion.div>
              )}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} disabled={formState === 'loading'} type="submit"
                  className={`w-full py-5 rounded-2xl font-bold text-xl flex items-center justify-center gap-3 transition-all shadow-xl ${formState === 'loading' ? 'bg-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-[#008737] to-[#085558] hover:shadow-2xl'}`}
                  style={{ color: '#ffffff' }}>
                  {formState === 'loading'
                    ? <div className="w-6 h-6 border-4 border-white/30 border-t-white rounded-full animate-spin"></div>
                    : <><Send className="h-5 w-5" style={{ color: '#ffffff' }} /><span style={{ color: '#ffffff' }}>Send Message</span></>
                  }
                </motion.button>
              </form>
            </div>
      ...
      <section className="pb-20 lg:pb-32 px-4 container mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} className="h-[400px] lg:h-[500px] bg-white rounded-[40px] shadow-2xl border-8 border-white overflow-hidden relative group">
          <div className="absolute inset-0 bg-gray-200 flex flex-col items-center justify-center text-center p-8">
            ...
            <button className="mt-8 px-8 py-3 bg-[#063630] text-white rounded-xl font-bold shadow-lg hover:bg-[#008737] transition-colors" style={{ color: '#ffffff' }}>
              Open in Google Maps
            </button>
          </div>
        </motion.div>
